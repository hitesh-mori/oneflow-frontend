...
import 'add_edit_purchase_order_dialog.dart';

class PurchaseOrdersContent extends StatefulWidget {
  const PurchaseOrdersContent({super.key});

  @override
  State<PurchaseOrdersContent> createState() => _PurchaseOrdersContentState();
}

class _PurchaseOrdersContentState extends State<PurchaseOrdersContent> {
  ...
  Future<void> _loadOrders() async {
    ...
    final result = await _orderService.getOrdersByType('Purchase');
    if (result['success']) {
      setState(() {
        _orders = result['data'];
      });
  ...
  Future<void> _showAddOrderDialog() async {
    final result = await showDialog(
      context: context,
      builder: (context) => const AddEditPurchaseOrderDialog(),
    );
    if (result == true) {
      _loadOrders();
    }
  ...
  Future<void> _showEditOrderDialog(OrderModel order) async {
    final result = await showDialog(
      context: context,
      builder: (context) => AddEditPurchaseOrderDialog(order: order),
    );
    if (result == true) {
      _loadOrders();
    }
