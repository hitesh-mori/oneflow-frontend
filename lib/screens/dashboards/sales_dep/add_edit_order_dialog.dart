...
class AddEditOrderDialog extends StatefulWidget {
  final OrderModel? order;

  const AddEditOrderDialog({super.key, this.order});

  @override
  State<AddEditOrderDialog> createState() => _AddEditOrderDialogState();
}

class _AddEditOrderDialogState extends State<AddEditOrderDialog> {
  ...
  void _initializeFromOrder() {
    ...
    _selectedCustomer = order.partner;
  ...
  Future<void> _loadCustomers() async {
    final result = await _partnerService.getPartnersByType('Customer');
    if (result['success']) {
      setState(() {
  ...
  Future<void> _loadProducts() async {
    final result = await _productService.getProductsByType('sale');
    if (result['success']) {
      setState(() {
  ...
  Future<void> _save() async {
    ...
    if (_selectedCustomer == null) {
      AppToast.showError(context, 'Please select a customer');
      return;
    }
    ...
    try {
      final orderData = OrderModel(
        id: widget.order?.id ?? '',
        code: widget.order?.code ?? 'New Sales Order',
        type: 'Sales',
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(
                        widget.order == null ? 'New Sales Order' : 'Edit Sales Order',
                        style: TextStyle(
                          fontSize: 24,
                  ...
                  : SingleChildScrollView(
                      child: Form(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            // Customer, Project, Date, and Status Row
                            Row(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Expanded(
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: CustomButton(
                    title: widget.order == null ? 'Create Order' : 'Save Changes',
                    onTap: _save,
